...
export const initializePusher = (userId: number) => {
  ...
  // Kiểm tra token trước khi initialize
  if (!token) {
    console.warn("Cannot initialize Pusher: No token found")
    return null
  }
  ...
  // Nếu đã có pusher instance, disconnect và tạo mới nếu user khác
  if (pusher) {
    pusher.disconnect()
    pusher = null
  }
  
  console.log("Initializing Pusher with key:", PUSHER_APP_KEY, "cluster:", PUSHER_CLUSTER, "userId:", userId)
