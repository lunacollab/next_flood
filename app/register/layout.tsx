import type { Metadata } from "next"

export const metadata: Metadata = {
  title: "Đăng ký - Hệ thống cảnh báo lũ lụt",
  description: "Đăng ký tài khoản mới cho hệ thống cảnh báo lũ lụt",
}

export default function RegisterLayout({
  children,
}: {
  children: React.ReactNode
}) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-background relative overflow-hidden py-12">
      {/* Background decoration */}
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20 dark:opacity-10"></div>
      <div className="absolute top-0 left-0 w-96 h-96 bg-blue-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float dark:bg-blue-800 dark:opacity-10"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float dark:bg-blue-700 dark:opacity-10" style={{ animationDelay: "2s" }}></div>
      {children}
    </div>
  )
}

